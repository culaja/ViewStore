...
var viewStore = ViewStoreCacheBuilder.New()
    .For(PostgresViewStoreBuilder.New()
        ...
        .WithTablePath(tablePath: "test.ten_documents_rolling")
        .ShouldAutoCreate(
...
for (var i = 1L;; ++i)
{
    var eventEnvelope = new ViewEnvelope($"{nameof(SampleView)}-{i%10}", new SampleView(i), GlobalVersion.Of(i), MetaData.New());
